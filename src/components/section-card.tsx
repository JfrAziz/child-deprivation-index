...
const cloudNarative = `z-50 text-neutral-content font-bold text-[clamp(0.7rem,1.5vw,1rem)]             
    bg-base-200/90 px-[clamp(1rem,6vw,2rem)] py-[clamp(1rem,5vw,1.8rem)] 
    max-w-[clamp(12rem,32vw,32rem)] rounded-[2em]
...
export const SectionCard = forwardRef<HTMLDivElement, SectionCardProps>(
  (props, ref) => (
    ...
    >
      {props.title && (
        <div className="!text-left text-[clamp(1rem,1.5vw,1.5rem)] font-black leading-[1.15]">
      ...
      <div className="mt-[0.5em] !text-left text-[clamp(0.8rem,1.5vw,1rem)] font-light">{props.children}</div>
    </div>
  )
);
